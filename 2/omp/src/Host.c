...
////////////////////////////////////////////////////////////////////////////////
void init_subdomain(_DOUBLE_ *h_s_uold, _DOUBLE_ *h_uold, unsigned int Nx, unsigned int Ny, unsigned int _Nz, unsigned int i)
{
	...
	for (unsigned int k = 1; k <= nz; k++)
	{
		for (unsigned int j = 1; j <= ny; j++)
		{
			for (unsigned int i = 1; i <= nx; i++)
			{
				...
				_DOUBLE_ analytical = (exp(-3*M_PI*M_PI*t) * INITIAL_DISTRIBUTION(i, j, k, h)) - uOld[idx];
