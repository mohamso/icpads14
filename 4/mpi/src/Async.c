  ...
  if (rank == 0)
  {
	...
	for(unsigned int iterations = 0; iterations < max_iters; iterations++)
	{
		...
		if (rank < numberOfProcesses-1)
		{
			...
			checkCuda(cudaMemcpy2DAsync(right_send_buffer, dt_size*(Nx+2), d_right_send_buffer, pitch_gc_bytes, dt_size*(Nx+2), (Ny+2)*(_GC_DEPTH), cudaMemcpyDefault, right_send_stream));
			checkCuda(cudaStreamSynchronize(right_send_stream));

			MPI_CHECK(MPI_Isend(right_send_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &right_send_request[rank]));
		}
		...
		if (rank > 0)
		{
			...
			checkCuda(cudaMemcpy2DAsync(left_send_buffer, dt_size*(Nx+2), d_left_send_buffer, pitch_gc_bytes, dt_size*(Nx+2), (Ny+2)*(_GC_DEPTH), cudaMemcpyDefault, left_send_stream));
			checkCuda(cudaStreamSynchronize(left_send_stream));

			MPI_CHECK(MPI_Isend(left_send_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank-1, 1, MPI_COMM_WORLD, &left_send_request[rank]));
		}
		...
		if (rank < numberOfProcesses-1)
		{
			MPI_CHECK(MPI_Irecv(right_receive_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank+1, 1, MPI_COMM_WORLD, &right_receive_request[rank]));
		}

		// Receive data from 1-3
		if (rank > 0)
		{
			MPI_CHECK(MPI_Irecv(left_receive_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, &left_receive_request[rank]));
		}
      ...
      MPI_CHECK(MPI_Wait(&right_receive_request[rank], MPI_STATUS_IGNORE));
			...
			checkCuda(cudaMemcpy2DAsync(d_right_receive_buffer, pitch_gc_bytes, right_receive_buffer, dt_size*(Nx+2), dt_size*(Nx+2), ((Ny+2)*(_GC_DEPTH)), cudaMemcpyDefault, right_receive_stream));
      ...
      MPI_CHECK(MPI_Wait(&left_receive_request[rank], MPI_STATUS_IGNORE));
			...
			checkCuda(cudaMemcpy2DAsync(d_left_receive_buffer, pitch_gc_bytes, left_receive_buffer, dt_size*(Nx+2), dt_size*(Nx+2), ((Ny+2)*(_GC_DEPTH)), cudaMemcpyDefault, left_receive_stream));
