 ...
 * 
 * Synchronous Multi-GPU code where the number of threads spawned equals the 
 * number of GPUs. All memory transfers are synchronous. This code corresponds 
 * to "MPI" results in Figure-8 in the paper.
 *
 * 
...
int main(int argc, char** argv)
{
  ...
  if (rank < numberOfProcesses-1) {
    ...
    checkCuda(cudaMemcpy2D(right_send_buffer, dt_size*(Nx+2), d_right_send_buffer, pitch_gc_bytes, dt_size*(Nx+2), (Ny+2)*(_GC_DEPTH), cudaMemcpyDefault));

    MPI_Isend(right_send_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &right_send_request[rank]);
  }
  ...
  if (rank > 0) {
    ...
    checkCuda(cudaMemcpy2D(left_send_buffer, dt_size*(Nx+2), d_left_send_buffer, pitch_gc_bytes, dt_size*(Nx+2), (Ny+2)*(_GC_DEPTH), cudaMemcpyDefault));

    MPI_Isend(left_send_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank-1, 1, MPI_COMM_WORLD, &left_send_request[rank]);
  }

  // Receive data from 0-2
  if (rank > 0) {
    MPI_Irecv(left_receive_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, &left_receive_request[rank]);
  }

  // Receive data from 0-2
  if (rank < numberOfProcesses-1) {
    MPI_Irecv(right_receive_buffer, (Nx+2)*(Ny+2)*(_GC_DEPTH), MPI_DOUBLE, rank+1, 1, MPI_COMM_WORLD, &right_receive_request[rank]);
  }

  // Receive data from 0-2
  if (rank > 0) {
    MPI_Wait(&left_receive_request[rank], MPI_STATUS_IGNORE);
    checkCuda(cudaMemcpy2D(d_left_receive_buffer, pitch_gc_bytes, left_receive_buffer, dt_size*(Nx+2), dt_size*(Nx+2), ((Ny+2)*(_GC_DEPTH)), cudaMemcpyDefault));
  ...
  if (rank < numberOfProcesses-1) {
    ...
    checkCuda(cudaMemcpy2D(d_right_receive_buffer, pitch_gc_bytes, right_receive_buffer, dt_size*(Nx+2), dt_size*(Nx+2), ((Ny+2)*(_GC_DEPTH)), cudaMemcpyDefault));
